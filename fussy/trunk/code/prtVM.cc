/* $Id: prtVM.cc,v 1.4 2006/08/06 22:05:06 sbhatnag Exp $ */
/******************************************************************
...
void showHelp()
{
  ...
  cerr << INDENT << "These variables can be set only by special commands" << endl
       << INDENT << "setlong and setlat (see below)." << endl;
  ...
  cerr << "\t" << "setlong(MyLongitude): Sets the global variables LONGITUDE" << endl
       << INDENT << "to the given value." << endl;
  cerr << "\t" << "setlat(MyLatitude): Sets the global variables LATITUDE to" << endl
       << INDENT << "the given value." << endl;
  
  cerr << endl << "Functions to set system variables:" << endl;
  cerr << "\t" << "setfmt(FMTSTR): Sets the system variable \'fmt\' used as" << endl
