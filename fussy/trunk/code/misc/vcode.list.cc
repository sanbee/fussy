//$Id: vcode.list.cc,v 1.1.1.1 2005/09/27 21:52:40 sbhatnag Exp $
/******************************************************************
 ...
 *
  ...
  History:
    ...
    principle, the code can now becomes much cleaner (then in the
    final analysis, implementing this was a liberating experience
    after all! :)).
    ...
    results (the author, YT, himself went on a wild chase for a bug
    ...
    of scope or not in use by any other symbol.
    ...
    All symbols with zero error also have an ID of zero.  This
    minimizes runtime memory requirements (further runtime performance
    improvement can be achieved by eliminating unnecessary stack
    operations).
...
template <class X> X& TOP(vector<X>& v) 
{
  if (!v.size()) 
    ReportErr("Illegal operation on stack or symbol table!\n            "
	      ...
	      "this is an internal error!","###Runtime",0);
  return v.back();
}
//
//-------------------------------------------------------------------
...
// currently pointing.
//
//  VMac& P    The VM program in memory.
//
NUMTYPE Run(VMac& P)
{
  Result=0;
...
//
//-----------------------------------------------------------------
// VM instruction: Perform a pop operation on the VM stack. 
//
int pop()
{
  StackType d;
...
//-----------------------------------------------------------------
//
//VM instruction: Push instruction to push a constant.  Used in
//suitations where error propagation computations are not required
//(like in relational operators, a<b, a>b, etc.).
//
int fcpush()
{
  StackType d;
...
//
//-----------------------------------------------------------------
// VM instruction: Push instruction to put a constant.
//
int cpush()
{
  StackType d;
...
//-----------------------------------------------------------------
//
// Function to emit VM instructions rvpush or vpush followed by the
// symbol to be pushed.
//
...
//-----------------------------------------------------------------
//
// VM instruction: Push a variable on the stack.  The pointer to the
// variable pushed on the stack is found in the next location in the
// VM program.
//
int vpush()
{
...
//-----------------------------------------------------------------
//
// VM instruction: Push a referenced variable on the stack.  The
// symbol to be pushed in this case if found at an offset in the local
// symbol table.  The offset is in the next location of the VM
// program.  This is required for the symbols in a sub-program scope.
//
int rvpush()
{
  StackType d;
...
//
//-----------------------------------------------------------------
// VM instruction: The NOP operator
//
int nop()
{
  StackType d1;
...
int add()
{
  ...
  //
  // Computation of partial derivatives w.r.t.
...
int mul()
{
  ...
  set_intersection(d1.ID.begin(), d1.ID.end(),
		   ...
		   inserter(res,res.end()));
  //
  // POP a value from the stacks corresponding to IDs in the common
  // set.
  //
  ...
  for (i=res.begin();i!=res.end();++i) 
    {
      ...
      //
      // Push the derivative on the stacks corresponding to IDs in the
      // common set.
      //
  ...
  set_symmetric_difference(d1.ID.begin(), d1.ID.end(),
			   ...
			   inserter(res,res.end()));
  //
  // Push the corresponding derivatives on the stacks corresponding to
  // the IDs in the unique ID set.
  //
#ifdef VERBOSE
  ...
  // set the resultant set as ID set of the result (d1).
  //
  res.clear();
  set_union(d1.ID.begin(), d1.ID.end(),
	    d2.ID.begin(), d2.ID.end(),
...
int vdiv()
{
  ...
  set_intersection(d1.ID.begin(), d1.ID.end(),
		   ...
		   inserter(res,res.end()));
  //
  // POP a value from the stacks corresponding to 
  // IDs in the common set.
  //
  for (i=res.begin();i!=res.end();++i) 
    {
      d2x = TOP(DS[(*i)]);   POP(DS[(*i)]);
  ...
  if (!(d1.val.val()==0))
    {    
      ...
      set_intersection(d1.ID.begin(), d1.ID.end(),
		       ...
		       inserter(res,res.end()));
      //
      // POP a value from the stacks corresponding to IDs in the
      // common set.
      //
      ...
      for (i=res.begin();i!=res.end();++i) 
	{
	  ...
	  //    }
	  //
	  // Push the derivative on the stacks corresponding to IDs in
	  // the common set.
	  //
      ...
      set_symmetric_difference(d1.ID.begin(), d1.ID.end(),
			       ...
			       inserter(res,res.end()));
      //
      // Push the corresponding derivatives on the stacks
      // corresponding to the IDs in the unique ID set.
      //
      for (i=res.begin();i!=res.end();++i)
	{
	  d1x = TOP(DS[(*i)]);   //POP(DS[(*i)]);
      ...
      // and set the resultant set as ID set of the result (d1).
      //
      res.clear();
      set_union(d1.ID.begin(), d1.ID.end(),
		d2.ID.begin(), d2.ID.end(),
...
int lst()
{
  ...
  //
  // Construct a new symbol of type RETVAR_TYPE and which is a 
  IDType i;
...
int assgn()
{
  ...
  //  if ((Type!=VAR) && (Type!=SYS_VAR) && (Type != PARTIALVAR_TYPE) && ((Type&AUTOVAR_TYPE)!=AUTOVAR_TYPE))
  //
  // Switch off the PARTIALVAR_TYPE bit and switch on the
  // DEFAULT_TYPE (VAR) bit.
  //
...
#endif
	  //
	  // symb->type.qstr is expected to be empty (==NULL).
	  // If d2 is a new symbol, install should have set it 
	  // to NULL.  If d2 is an existing VAR from the Symbol
	  // table of type QSTRING, the grammar rule
	  //    QSTRING '=' expr 
	  // will delete any existing qstr.
	  // If a non-QSTRING variable is getting converted to
	  // QSTRING, it will already have qstr=NULL (since it
	  // was created by install).
	  // Hopefully, this should take care of any mem-leak.
	  // Hopefully!
	  //
  ...
  if (d2.symb) ERROUT << "assgn(): RHS types:" << endl;prtTypes(d2);
    ...
    //    if (!ISSET(d2.symb->type,PARTIALVAR_TYPE))
...
int passgn()
{
  ...
  //
  // Release the ID associated with this symbol since it's now a
  // partial variable (a sub-tree) and hence does not have an identity
  // of it's own.
  //
...
int ifcode()
{
  ...
  d=TOP(stck);   POP(stck);
  //
  // POP the derivative from the appropriate DS
  // for the result of the condition code
  //
  ClearDS(d);
...
int whilecode()
{
  ...
  Run(Prog);
  d=TOP(stck);   POP(stck);
  if (ISSET(d.symb->type, RETVAR_TYPE)) IDR.ReleaseID((unsigned int)*(d.ID.begin()));
...
{
  //
  // Execution of the for(INIT;CONDITION;PREDICATE) BODY;
  // loop
  //
...
int printcode()
{
  ...
  // Result (which is returned to the caller of top level API as the
  // final result).
  //
  Result=d.val;
...
#endif
  //
  // Release everything in the TmpSymbTab irrespective of 
  // whether it is in a permanent symb. table or not.
  //
  LetGoID(d,RETVAR_TYPE,1,0);
...
int call()
{
  ...
  try
	...
	ReportErr("Program conuter gets invalid value for a function call",
      ...
      // arguments declared in the sub-prog. code
      //
      NArgs  = (int)Prog[pc++]; 
      NAutos = (int)Prog[pc++];
	...
	ReportErr("Too many arguments in function call!","###Runtime",0);
      ...
      // over SP+N-1 locations on the LocalSymbTab
      //
  ...
  catch (ReturnException x)
    {
      ...
      //
      // Remove the symbols local to the current scope
      //
...
int ret()
{
  ...
  // from a function sub-program.  Create a symbol on the TmpSymbTab,
  // pop the VMS and package its contents in the new symbol, and push
  // it back on the VMS.  This re-packaging is required since the
  // returned value could have been constructed out of auto variables,
  // which will disappear immediately after the return instruction is
  // executed and hence not valid for consumption later.
  //
  ...
  if (stck.size() > 0)
    {
      ...
      // (auto) variable, replace the top of the object on the top of
      // the VM stack by a new object which has the same value but a
      // new ID.  The ID associated with the variable which on the top
      // of the stack will be released in the clean up process in
      // call() (the ReturnExecption takes the execution exactly
      // there).
      //
      d = TOP(stck);      POP(stck);
      ...
      if (d.symb)  
	{
      ...
      if ((d.symb!=NULL))
	//	  &&  (ISSET(d.symb->type,PARTIALVAR_TYPE|RETVAR_TYPE|AUTOVAR_TYPE)))
	  //	  && (!ISSET(d.symb->type,AUTOVAR_TYPE)))
      //      if (ISSET(d.type,PARTIALVAR_TYPE))
	{
	  N=d.ID.size();
      ...
      else
	{
      ...
      int k=0;
      //      if ((r.symb!=NULL) && ISSET(r.symb->type,PARTIALVAR_TYPE))
      //      if (ISSET(r.symb->type,PARTIALVAR_TYPE))
      if(1)
	//      if (ISSET(d.symb->type,PARTIALVAR_TYPE|RETVAR_TYPE|AUTOVAR_TYPE))
	//	  && !ISSET(d.symb->type,AUTOVAR_TYPE))
	for (IDList::iterator ii=r.symb->IDL.begin();ii!=r.symb->IDL.end();++ii)
	  //	for (IDList::iterator ii=r.ID.begin();ii!=r.ID.end();++ii)
	  {
  ...
  //
  // Throw a ReturnException.  This is only caught in call(), where
  // the local symbol table is cleaned, the FrameStack is poped and
  // the SP, PC pointers are loaded from the Frame stack.
  //
#ifdef VERBOSE
  prtStacks("Ret out");
#endif
