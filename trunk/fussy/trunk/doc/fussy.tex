...
\maketitle
\begin{center}
\htmladdnormallinkfoot{[Postscript version]}{http://www.aoc.nrao.edu/~sbhatnag/Softwares/fussy/fussy.ps}
\end{center}
...
The \Fussy\footnote{The name reflects the original intention of designing a language for fuzzy arithemetic.  It is also a pun on those who (wrongly) consider error propagation as too much fuss!} scripting language, described here, implements an
...
an interpreter by the user.  A simple {\tt C} binding to the
interpreter is also provided.  Mathematical expressions can be
