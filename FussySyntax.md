...
Numbers and variables can be combined with the mathematical operators and logical operators to form an expression. Expressions can be used as arguments to built-in or user defined functions (see Section [Function/procedure](#functionprocedure)). An expression followed by a NEWLINE prints its result on the output stream (see Section [Print Statement](#print-statement)) in the default format (see Section [Formatting](#formatting)).

For the purpose of error propagation, the print statement and the assignment operator (the "='' operator but not the ":='' operator; see Section on [Sub-expressions](#sub-expressions)) are treated as the terminal nodes of the parsing tree which invokes the final error propagation.
